                 ...
                 out.println("<br/>PLEASE ENTER A CORRECT TIME!");
                    ...
                    Connection con=null;
                     ...
                     //if all conditions satisfy ALL GOOODD YYYAAAAYYY ::)))) //i got lost from all the if X(
                      ...
                      if(Doctor.isAppRep(appdate, apptime,nid) > 0){
                          request.getRequestDispatcher(errorPath).include(request,response);
                          out.println("<br/>Error You cannot time is already booked!");
                      }
                      else if(Doctor.addApp(spec, appdate, nid, appPrice, apptime, isOnine, clinicName, applink) > 0)
